...
O worker sob demanda foi **publicado em `silogium-authoring` no environment `main`**, em 10/09/2026. As migrações `202609100014_beta_access_and_creation_quota.sql` e `202609100015_operational_capacity.sql` também foram aplicadas; 400 pgTAP passaram contra o schema real em transações revertidas. O smoke remoto comprovou wake sem token/ inválido (401), corpo não vazio (400) e notificação autenticada vazia (202 `idle`), incluindo leitura de consumo no contexto nativo do container, sem token pessoal Modal. **Uma criação real privada clássica TypeScript passou até validação**, com confirmação de similares, checkpoints e consumo único da cota diária. Isso não é o benchmark de 40 prompts nem certificação de morte de container/OOM. Conferir o estado da web em [DEPLOYMENT](./DEPLOYMENT.md). A configuração de IA é [Groq](./groq-integration.md).
